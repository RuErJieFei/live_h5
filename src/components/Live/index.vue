<template>
  <div class="container">
    <div class="sidebar">
      ...
      <a class="logo-expand" href="#">智慧南工</a>
    ...
    <div class="wrapper">
      <div class="header">
        <div class="search-bar">
          <input type="text" placeholder="输入直播间号">
        </div>
      ...
      <div class="main-container">
        <div class="main-header anim" style="--delay: 0s">发现</div>
        <div class="videos">
          <div class="video anim" style="--delay: .4s" v-for="(room, index) in rooms" :key="room.id"
               @click="play(index)">
            ...
            <div class="video-wrapper">
              ...
              <div class="author-img__wrapper video-author">
                ...
                <img class="author-img" :src="room.ownerAvatar"/>
              </div>
            </div>
            <div class="video-by">{{ room.ownerName }}</div>
            <div class="video-name">{{ room.roomName }}</div>
            <div class="video-view">54K views<span class="seperate video-seperate"></span>1 week ago</div>
          </div>
        </div>
        <div class="stream-area">
          <div class="video-stream">
            <video name="videoElement" class="video-js vjs-default-skin anim" width="640px" height="267px" controls
                   autoplay>
              浏览器版本过低，不支持 HTML5 video.
            </video>
            <el-button @click="flvLoad" class="refresh_button">
              <img src="@/assets/refresh.png" style="width: 20px">
            </el-button>
            <div class="video-detail">
              <div class="video-content">
                <div class="video-p-wrapper anim" style="--delay: .1s">
                  <div class="author-img__wrapper video-author video-p">
                    ...
                    <img class="author-img"
                         src="https://ruerjiefei.oss-cn-hangzhou.aliyuncs.com/images/201125DN3SDB9SK4.JPG"/>
                  </div>
                  <div class="video-p-detail">
                    <div class="video-p-name">簇簇</div>
                ...
                <!--                <div class="video-p-title anim" style="&#45;&#45;delay: .2s">{{ currentRoom.roomName }}</div>-->
                <!--                <div class="video-p-subtitle anim" style="&#45;&#45;delay: .3s">{{ currentRoom.type }}</div>-->
                <!--                <div class="video-p-subtitle anim" style="&#45;&#45;delay: .4s"></div>-->
              </div>
            </div>
          </div>
          <!--    聊天      -->
          <div class="chat-stream">
            <div class="chat">
              <div class="chat-header anim">聊天<span><svg viewBox="0 0 24 24" fill="currentColor"
         ...
         <path fill-rule="evenodd" clip-rule="evenodd"
              ...
              <div class="message-container">
                <div class="message anim" style="--delay: .1s" v-for="message of messageList" :key="message.id">
                  <div class="author-img__wrapper video-author video-p">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"
                         stroke-linejoin="round" class="feather feather-check">
                      <path d="M20 6L9 17l-5-5"/>
                  ...
                  <div class="msg-wrapper">
                    <div class="msg__name video-p-name"> {{ message.username }}</div>
                    <div class="msg__content video-p-sub"> {{ message.content }}</div>
                  </div>
                </div>
              </div>
              <div class="chat-footer anim" style="--delay: .1s">
                <input v-model="content" @keyup.enter="sendMsg" type="text" placeholder="请输入内容">
              </div>
            </div>
            <div class="chat-vid__container">
              ...
              <div class="chat-vid anim" style="--delay: .4s">
                <div class="chat-vid__wrapper">
                  <img class="chat-vid__img" src="http://124.221.232.15:9090/hlh/Snipaste_2022-11-30_14-38-42.png"/>
                  <div class="chat-vid__content">
                    <div class="chat-vid__name">劲松的数学课</div>
                    <div class="chat-vid__by">劲松</div>
                    <div class="chat-vid__info">125.908 views <span class="seperate"></span></div>
                  </div>
                </div>
              </div>
              <div class="chat-vid anim" style="--delay: .5s">
                <div class="chat-vid__wrapper">
                  <img class="chat-vid__img"
                       src="http://124.221.232.15:9090/hlh/Snipaste_2022-11-30_14-31-43.png"/>
                  <div class="chat-vid__content">
                    <div class="chat-vid__name">珍芳的英语课</div>
                    <div class="chat-vid__by">珍芳</div>
                    <div class="chat-vid__info">125.908 views <span class="seperate"></span></div>
                  </div>
                </div>
              </div>
              <div class="chat-vid__button anim" style="--delay: .6s">查看相关 (32)</div>
            </div>
          </div>
        </div>
      </div>
...
import {liveUrl, loginUrl} from "@/config/config";
...
const fromId = JSON.parse(localStorage.getItem('info')).id
...
const wsMessage = (data) => {
  console.log(data)
  console.log(data.msg.roomId === roomId.value)
  // 按照房间区分
  if (data.msg.roomId === roomId.value) {
    // 这里写拿到数据后的业务代码
...
let user = ref({})
const getUser = async () => {
  const res = await axios.get(loginUrl + "/user", {
    params: {
      userId: fromId
    }
  })
...
const url = ref("http://101.200.120.36:8087/live/")
let key = ref("rfBd56ti2SMtYvSgD5xAV0YU99zampta7Z7S575KLkIZ9PYk")
let roomId = ref("")
const flvLoad = () => {
...
const roomList = async () => {
  const res = await axios.get(liveUrl + "/room", {
    params: {
      page: 0,
      size: 10
    }
  ...
  if (res.data) {
    res.data.data.forEach((e) => rooms.value.push(e))
    roomMain.value = rooms.value[0]
    roomSec.value = rooms.value[1]
  }
}

const play = (index) => {
  $(".discover").removeClass("is-active");
  $(".trending").addClass("is-active");
  $(".main-container").addClass("show");
  $(".main-container").scrollTop(0);
  key.value = rooms.value[index].roomKey
  roomId.value = rooms.value[index].roomId
  flvLoad();
}
...
:root {
  ...
  --body-color: #808191;
  --button-bg: #353340;
  --border-color: rgb(128 129 145 0.24);
  --video-bg: #252936;
  --delay: 0s;
}
